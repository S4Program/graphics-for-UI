...
:
UIElement(size),
cursor(sf::Vector2f(size.y, size.y), foreColor, window)
{
...
void gui::Slider::update()
{
    ...
    if(cursor.isPressed(sf::Mouse::Button::Left) || isSelected && cursor.getMouseHandle().isPressed(sf::Mouse::Button::Left))
    {
    ...
    else if(!cursor.getMouseHandle().isPressed(sf::Mouse::Button::Left))
    {
        isSelected = false;
    }
}